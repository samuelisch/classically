...
const WorkPageRecordingList = ({ selectedComposer, work }: PropTypes) => {
  ...
  useEffect(() => {
    ...
    if (selectedComposer.name.length && work) {
      spotifyCall
        .searchTrack(
          encodeURIComponent(normalizeString(selectedComposer.complete_name)),
          encodeURIComponent(normalizeString(work.searchterms[0]))
        )
        .then((response) => {
          if (mounted) {
            const tracks = response.tracks.items;
            const formattedTracks = tracks.map(
              (track: any): RecordingPropsType => {
                return {
                  id: track.id,
